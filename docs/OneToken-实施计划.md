...
| 2026-08-06 | **参考不指定 OpenRouter（用户裁决）**：设计 v0.17（§7.2 参考=第一方官方 API、聚合器仅审计目标、§7.3 决策表、§9.2 试点、头部决策）；冒烟记录 §3 试点指引与三协议 enroll 表述同步；M2.7 任务行措辞更新 | 助手 |
...
| 2026-08-06 | **用户裁决：参考不指定 OpenRouter（v0.17）**——参考指纹只用厂商第一方官方 API；聚合器（OpenRouter 等）多上游路由致参考分布不稳定（R4），不作参考来源、仅作审计目标；设计 §7.2/§7.3/§9.2、冒烟记录 §3、M2.7 试点表述同步（enroll 一律第一方 provider；`--provider openrouter` 仅 audit/probe） | 用户意见 |
