...
| R7 | 协议差异：o 系拒绝 effort 最低档、推理模型不可直接指纹化、Azure 形态特殊 | 两套系统（v0.19）：非推理走论文方法，推理走 post-reasoning 通道（M2.9，DeepSeek 实测可行）；Azure 移 v1.1 | 开放 |
...
| 2026-08-06 | **推理通道稳定性实验 + 设计 v0.20**：§1.2/§3.4/§7/§9.2 补稳定性数据（同模型 0.1116、跨模型 0.238、τ 区间 0.15–0.18、k≥16 建议）；冒烟记录 §7 完整实验记录 | 助手 |
| 2026-08-06 | **两套系统决策（用户裁决 + DeepSeek 实测）**：设计 v0.19（§1.2 post-reasoning 升级系统 2、§3.1 采集参数按通道、§5 hidden-reasoning 分流语义、§7 通道分档 reasoning、§9.2 试点数据）；实施计划新增 M2.9 推理通道适配；风险 R7 缓解扩展（推理模型不再纯排除，走系统 2） | 助手 |
...
| 2026-08-06 | **推理通道稳定性实验通过（用户要求先验证再推进，v0.20）**：同模型两次独立采集 0.1116 ≈ 分裂半基线 0.09–0.10（探测几乎不变）、跨模型 0.2376/0.2382 可复现（2 倍分离）；τ 区间 0.15–0.18、审计 k≥16 建议入设计 §3.4；M2.9 实现 + 关键修复（Text 管线）完成，正式校准待办 | 用户意见 + 实测 |
| 2026-08-06 | **用户裁决：两套系统（v0.19）**——保留论文原方法（系统 1：非推理 direct 通道）+ 新增推理通道（系统 2：post-reasoning 回答指纹）；**DeepSeek 实测证实可行性**：v4-flash/pro 均为推理模型（reasoning_tokens 占满 16 预算），max_tokens=512 时拿到思考后回答（reasoning=35+answer、finish=stop），flash 偏好 42 / pro 偏好 73（8 次采样分布可区分）；设计 §1.2/§3.1/§5/§7/§9.2 同步；新增 M2.9 推理通道适配任务（探测器分流、采集参数按通道、τ 单独校准、同通道比对） | 用户意见 + 实测 |
...
| 2026-08-09 | **M2.10 compare 直比模式完成（✅）**：CLI compare + HTML/JSON 输出 + 内置参考线（TauSource）+ MemoryStore/ResponseSink + --save-ref/--save-responses；mock e2e 6 用例 + 真实 mock 冒烟（同模型 pass score 0.0404/12 cell、冒充 suspicious score 1.0、内置线 0.14、stdout 仅摘要、默认不落库仅 reports/、HTML 报告含判定徽标/未校准警示/分布对比/QC）；新增 17 测试（verify +2、store +3、report +6、compare e2e +6）；**pre-existing 修复**：ratelimit 假时钟时间炸弹（rpmRef 与假时钟不同源致 el<0 死循环——2026-08-09 环境日期越过 2026-08-06 硬编码起点后暴露，ref 重置到假时钟起点）；compare 参考端有效 cell<k_min 友好诊断；e2e flag 污染 resetCompareFlags；**三视角审查后修复**：--save-ref 版本冲突预检提前到采集前（防 240+ 次 API 浪费）、--ref-model-id/--ref-version 控制字符过滤（ANSI 终端注入）、测量短路 stdout 显示「测量无效，未判定」非 τ=0.0000、--tau 补 >1 校验（audit 同步）、报告文件名随机后缀防同秒覆盖；设计 §8 CLI 示例补 --ref-model-id；未采纳：TargetChannel=base_url 致校准分支不可达（设计预期，fail-safe）、TaskForCell 副本重复（刻意）、进度条交错（外观）；真实试点（M2.8/§9.2 compare 试点）待密钥 | 助手 |
